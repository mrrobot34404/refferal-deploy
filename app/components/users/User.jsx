  ...
  const [users, setUsers] = useState(user);
    ...
    const togglePaymentStatus = (id) => {
        ...
        const updatedUsers = users.map(user => 
            user.id === id ? { ...user, status: user.status === 'Paid' ? 'Unpaid' : 'Paid' } : user
          );
    ...
    return (
        <section className="py-8 md:ml-[320px] mt-8">
          {users.map(data => (
    <div className="container px-4 mx-auto" key={data.id}>
      <div className="p-4 mb-6 bg-white shadow rounded overflow-x-auto">
        <table className="table-auto w-full">
          <thead>
            <tr className="text-xs text-gray-500 text-left"><th className="pl-6 pb-3 font-medium">Customer ID</th><th className="pb-3 font-medium">User</th><th className="pb-3 font-medium">Joined</th><th className="pb-3 font-medium">Status</th><th className="pb-3 font-medium">Level</th><th className="pb-3 font-medium">Action</th></tr>
          </thead>
          <tbody>
            <tr className="text-xs bg-gray-50">
              <td className="py-5 px-6 font-medium">#{data.id}</td>
              ...
              <td>
                       ...
                       <span
                        onClick={() => togglePaymentStatus(data.id)}
              ...
              <td>
                <span className="inline-block py-1 px-2 text-purple-500 bg-purple-50 rounded-full">{data.level}</span>
              </td>
              <td>
